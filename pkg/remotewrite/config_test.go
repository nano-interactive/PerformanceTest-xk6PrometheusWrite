...
func TestConfigParseArg(t *testing.T) {
	...
	c, err = ParseArg("writeMetrics=one|two|three")
	assert.Nil(t, err)
	assert.Equal(t, true, c.WriteMetrics["one"])
	assert.Equal(t, true, c.WriteMetrics["two"])
	assert.Equal(t, true, c.WriteMetrics["three"])
