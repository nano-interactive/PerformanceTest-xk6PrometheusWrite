...
type Config struct {
	...
	WriteMetrics     map[string]bool `json:"writeMetrics" envconfig:"K6_NANO_WRITE_METRICS"`
	HistogramBuckets []float64       `json:"histogram-buckets" envconfig:"K6_NANO_HISTOGRAM_BUCKETS"`
...
func ParseArg(arg string) (Config, error) {
	...
	if v, ok := params["writeMetrics"].(string); ok {
		if c.WriteMetrics == nil {
			c.WriteMetrics = make(map[string]bool)
		}
		for _, v := range strings.Split(v, "|") {
			if len(v) > 0 {
				c.WriteMetrics[v] = true
			}
		}
	}
...
func GetConsolidatedConfig(jsonRawConf json.RawMessage, env map[string]string, arg string) (Config, error) {
	...
	getEnvMapStringSlice := func(env map[string]string, name string) []string {
		var result []string
		if v, vDefined := env[name]; vDefined {
	...
	getEnvFloat64Slice := func(env map[string]string, name string) []float64 {
		var result []float64
		if v, vDefined := env[name]; vDefined {
			tmpResult := strings.Split(v, ",")
			for _, tmpResult := range tmpResult {
				v, err := strconv.ParseFloat(tmpResult, 64)
				if err != nil {
					return []float64{}
				}
	...
	if urlLocal, urlDefined := env["K6_PROMETHEUS_REMOTE_URL"]; urlDefined {
		result.Url = null.StringFrom(urlLocal)
	}
	...
	envWriteMetrics := getEnvMapStringSlice(env, "K6_NANO_WRITE_METRICS")
	if len(envWriteMetrics) > 0 {
		// delete default metrics since we have a value
		result.WriteMetrics = make(map[string]bool)

		for _, v := range envWriteMetrics {
			result.WriteMetrics[v] = true
		}
	}

	envHistogramBuckets := getEnvFloat64Slice(env, "K6_NANO_HISTOGRAM_BUCKETS")
	if len(envHistogramBuckets) > 0 {
		result.HistogramBuckets = envHistogramBuckets
	}
